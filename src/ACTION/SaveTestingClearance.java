...
public String saveData()
{
	...
	{
		//update testing status and end date
	...
	{ 
		...
		// String notification=notificationStage.trim();
		// Criteria cr1=session.createCriteria(FurnishingStageMaster.class);
		 //   cr1.add(Restrictions.eq("coachType",coach));
		 //   cr1.add(Restrictions.eq("substageDescription", notification));
		   // FurnishingStageMaster furnish=new FurnishingStageMaster();
		   // System.out.println(coach+notification);
	...
	//System.out.println("subtest method");
		...
		else
		{
			for(int j=0;j<previousStages.length;j++){
                  if(previousStages[j].equals(stageid))
                  {
                	  return true;
                  }
                  else
